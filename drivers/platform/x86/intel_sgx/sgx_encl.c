...
void sgx_encl_block(struct sgx_encl_page *encl_page)
{
	...
	zap_vma_ptes(vma, addr, PAGE_SIZE);
	ptr = sgx_get_page(encl_page->epc_page);
	ret = __eblock(ptr);
	sgx_put_page(ptr);
	if (ret) {
			     ...
			     bool pcmd)
{
	...
	index = SGX_ENCL_PAGE_BACKING_INDEX(entry, encl);
	if (pcmd)
		index >>= 5;
 ...
 * @backing:	a backing file
 * @pcmd:	a pcmd backing file
 *
...
int sgx_encl_load_page(struct sgx_encl_page *encl_page,
		       struct sgx_epc_page *epc_page,
		       struct page *backing, struct page *pcmd)
{
	unsigned long addr = SGX_ENCL_PAGE_ADDR(encl_page);
	struct sgx_encl *encl = encl_page->encl;
	struct sgx_pageinfo pginfo;
