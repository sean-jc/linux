...
static void kvm_queue_exception_vmexit(struct kvm_vcpu *vcpu, unsigned vector,
...
static int inject_pending_event(struct kvm_vcpu *vcpu, bool *req_immediate_exit)
{
	 ...
	 * or force an immediate re-entry and exit to/from L2.
	 */
	WARN_ON_ONCE(vcpu->arch.exception_vmexit.pending);
	...
	if (vcpu->arch.exception.pending) {
		 ...
		 * leave RF as-is.
		 *
		 * Note, some versions of Intel's SDM incorrectly describe the
		 * behavior of General Detect #DBs, which are fault-like.  They
		 * do _not_ set RF, a la code breakpoints.
		 */
