...
static void avic_set_pi_irte_mode(struct kvm_vcpu *vcpu, bool activate)
{
	...
	list_for_each_entry(ir, &svm->ir_list, node) {
		if (activate)
			WARN_ON_ONCE(amd_iommu_activate_guest_mode(vcpu->cpu, ir->data));
		else
			...
			struct kvm_vcpu *vcpu, u32 vector)
{
	...
	if (vcpu) {
		 ...
		 * the IRTE metadata so that it it can be converted to guest
		 * mode if AVIC is enabled/uninhibited in the future.
		 */
