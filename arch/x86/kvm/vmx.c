...
static struct sgx_epc_page_ops vmx_epc_ops;

static int __vmx_epc_fault(struct vmx_epc *epc, struct vm_fault *vmf)
{
	...
	gfn = PFN_DOWN(epc->base + (vmf->address - vmf->vma->vm_start));
	...
	ret = vm_insert_pfn(vmf->vma, vmf->address, SGX_EPC_PFN(epc_page));
	if (unlikely(ret)) {
...
retry:
	...
	if (ret == -EBUSY) {
		...
		/*
		 * Abandon ship if someone is attempting to write-lock
		 * mmap_sem.  We already hold a read-lock on mmap-sem,
		 * i.e. down_read_trylock() will fail iff there is a
		 * pending write-lock.
		 */
		if (unlikely(!down_read_trylock(&epc->mm->mmap_sem)))
			return VM_FAULT_SIGBUS;
...
static void vmx_epc_destroy(struct kvm *kvm)
{
	struct kvm_vmx *kvm_vmx = to_kvm_vmx(kvm);
	struct vmx_epc *epc = to_vmx_epc(kvm);
	...
	radix_tree_for_each_slot(slot, &epc->page_tree, &iter, 0) {
		page = *slot;
		if (page->epc_page) {
			if (sgx_free_page(page->epc_page))
				list_add(&page->epc_page->list, &secs_pages);
		}
		kfree(page);
	 ...
	 */
	list_for_each_entry(epc_page, &secs_pages, list)
		WARN_ON(sgx_free_page(epc_page));
...
static void skip_emulated_instruction(struct kvm_vcpu *vcpu)
{
	...
	WARN(to_vmx(vcpu)->sgx_enclave_exit,
		"KVM: skipping instruction after exit from SGX enclave");
...
/*
 * Verify the VCPU's SGX CPUID leafs, and initialize the guest's
 * virtual EPC if necessary.  Initializating the virtual EPC via
 * CPUID avoids a new IOCTL, and ensures the guest CPUID info is
 * consistent with the virtual EPC and across VCPUs.
 */
static int vmx_cpuid_update_sgx(struct kvm_vcpu *vcpu)
{
	...
	/*
	 * EPC cannot exist without SGX being supported on all
	 * processors, i.e. !SGX && EPC is an invalid model.
	 */
	if (epc && !sgx_allowed_in_guest(vcpu))
		return -EINVAL;

	/*
	 * Always update the ENCLS bitmap, it's not safe to assume
	 ...
	 * via IA32_FEATURE_CONTROL bit 18.
	 */
	...
	if (!best || !(best->eax & bit(X86_FEATURE_SGX1)))
		return -EINVAL;

	/*
	 * SGX without SECS.ATTRIBUTES (CPUID.0x12.0x1) is an invalid
	 * model.  Even though we effectively ignore what is reported
	 * to the guest for the allowed SECS.ATTRIBUTES (see below),
	 * we at least want to guarantee the guest sees a valid model.
	 * The guest's allowed SECS.ATTRIBUTES are ignored because we
	 * we don't trap ECREATE, e.g. if supported by the platform,
	 * the guest can ECREATE a 64-bit enclave regardless of what
	 * CPUID reports.
	 */
	...
	if (best) {
		best->eax &= 0xf;
		if (epc)
			best->eax |= (uint32_t)(epc->base & 0xfffff000);
		best->ebx = epc ? (uint32_t)(epc->base >> 32) : 0;
		best->ecx &= 0xf;
		if (epc)
...
static int vmx_cpuid_update(struct kvm_vcpu *vcpu)
{
	...
	return vmx_cpuid_update_sgx(vcpu);
}
