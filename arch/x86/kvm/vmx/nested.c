 ...
 * been modified by the L1 guest.  Note that although the "read-only" VM-exit
 * information fields are actually writable if the vCPU is configured to
 * support "VMWRITE to any supported field in the VMCS", KVM always intercepts
 * and emulates VMWRITEs to those fields.
 */
...
static void prepare_vmcs02_full(struct kvm_vcpu *vcpu, struct vmcs12 *vmcs12)
{
	...
	if (!hv_evmcs || !(hv_evmcs->hv_clean_fields &
		...
		if (enable_ept && !is_long_mode(vcpu) && is_pae(vcpu) &&
		    is_paging(vcpu)) {
...
static int handle_vmwrite(struct kvm_vcpu *vcpu)
{
	...
	if (!is_guest_mode(vcpu)) {
		...
		/*
		 * Ensure vmcs12 is up-to-date before VMWRITE that dirties
