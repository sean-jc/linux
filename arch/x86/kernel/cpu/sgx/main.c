...
/**
 * sgx_record_page() - Add a page to the LRU tracking
 * @page:	EPC page
 ...
 * (un)reclaimable list.
 */
void sgx_record_page(struct sgx_epc_page *page, unsigned long flags)
{
...
/**
 * sgx_drop_page() - Remove a page from a LRU list
 * @page:	EPC page
 *
 ...
 */
int sgx_drop_page(struct sgx_epc_page *page)
{
	struct sgx_epc_lru *lru = sgx_lru(page);
				...
				struct sgx_backing *backing)
{
	...
	if (!encl->secs_child_cnt) {
		if (atomic_read(&encl->flags) & SGX_ENCL_DEAD) {
			sgx_drop_page(encl->secs.epc_page);
		      ...
		      struct sgx_epc_cgroup *epc_cg)
{
	...
	int ret;
        ...
        if (!IS_ENABLED(CONFIG_CGROUP_SGX_EPC) || !epc_cg)
                sgx_isolate_pages(&sgx_global_lru, &nr_to_scan, &iso);
...
static void sgx_oom_encl(struct sgx_encl *encl)
{
	...
	do {
		...
		list_for_each_entry_rcu(encl_mm, &encl->mm_list, list) {
			...
			sgx_oom_zap(encl, encl_mm->mm, encl->base,
...
/**
 * sgx_can_reclaim() - Query whether global reclaim is possible.
 *
 * Return: %true if reclaim is possible, i.e. pages are on the reclaimable LRU.
 */
...
static int ksgxswapd(void *p)
{
	...
	while (!kthread_should_stop()) {
		...
		if (sgx_should_reclaim(SGX_NR_HIGH_PAGES))
			sgx_reclaim_pages(SGX_NR_TO_SCAN, false, NULL);
	}
