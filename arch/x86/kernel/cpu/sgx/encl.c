						...
						unsigned long addr)
{
	...
	if (!(encl->secs.epc_page)) {
		...
		sgx_record_page(epc_page, SGX_EPC_PAGE_ENCLAVE);
	}
	...
	encl->secs_child_cnt++;
	sgx_record_page(entry->epc_page, SGX_EPC_PAGE_ENCLAVE_RECLAIMABLE);
...
void sgx_encl_destroy(struct sgx_encl *encl)
{
	...
	radix_tree_for_each_slot(slot, &encl->page_tree, &iter, 0) {
		...
		if (entry->epc_page) {
			 ...
			 */
			if (sgx_drop_page(entry->epc_page))
				continue;
	...
	while (!list_empty(&encl->va_pages)) {
		...
		sgx_drop_page(va_page->epc_page);
...
struct sgx_epc_page *sgx_alloc_va_page(struct sgx_encl *encl)
{
	...
	epc_page->owner = encl;
	sgx_record_page(epc_page, SGX_EPC_PAGE_VERSION_ARRAY);
